...
class LoginController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        $oldRecords = DB::connection('mysql')->select("select count(*) from users where email = '" . $id . "'");
        ...
        DB::connection("mysql")->insert("insert into users (email,password) values ('" . $id . "','" . $password . "')");

        // ログイン id を取得するため、保存したレコード情報を取得する。
        $records = DB::connection('mysql')->select("select * from users where email = '" . $id . "'");
    ...
    public function sign_in(Request $request)
    {
        ...
        $records = DB::connection('mysql')->select("select * from users where email = '" . $id . "' and password = '" . $password . "'");
        if (count($records) == 0) {
            return response("処理中に問題が発生しました。<a href='/login'>前のページへ戻る</a>");
        }
